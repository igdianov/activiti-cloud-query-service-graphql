...
public class ReactorSubscribableChannel implements BeanNameAware, MessageChannel, SubscribableChannel {
    ...
    private final Map<MessageHandler, Disposable>
    ...
    public ReactorSubscribableChannel(boolean singleThreadedProducer) {
        ...
        if (singleThreadedProducer) {
            ...
            this.processor = TopicProcessor.<Message<?>>builder().share(true).bufferSize(Integer.MAX_VALUE).build();
        }
    }
    ...
    public boolean subscribe(final MessageHandler handler) {
        ...
        Disposable c = Flux.from(processor).subscribe(consumer);
    ...
    public boolean unsubscribe(MessageHandler handler) {
        Disposable control = messageHandlerConsumers.remove(handler);
        if (null == control) {
            return false;
        }
        control.dispose();
